#!/usr/bin/env python3
"""
PostgreSQL HR Seed Data Generator for noui-connector-lab.

Creates ERPNext-compatible HR tables in PostgreSQL and populates them with
the same test data structure as the ERPNext/MariaDB target. This validates
the connector's PostgreSQL adapters against a live database.
...
Usage:
    ...
    python seed.py
    python seed.py --host 127.0.0.1 --port 5433 --user hrlab --password hrlab --database hrlab
"""
...
# ---------------------------------------------------------------------------
# Constants (same as ERPNext seeder)
# ---------------------------------------------------------------------------
COMPANY = "NoUI Labs"
COMPANY_ABBR = "NL"
CURRENCY = "USD"
...
def batch_insert(cursor, table, columns, rows, batch_size=500):
    """Insert rows in batches using PostgreSQL syntax."""
    if not rows:
        return
...
# ---------------------------------------------------------------------------
SCHEMA_DDL = """
-- ERPNext-compatible HR schema for PostgreSQL adapter validation.
-- Table names use "tab" prefix to match ERPNext conventions.

DROP TABLE IF EXISTS "tabAttendance" CASCADE;
DROP TABLE IF EXISTS "tabLeave Application" CASCADE;
DROP TABLE IF EXISTS "tabLeave Allocation" CASCADE;
DROP TABLE IF EXISTS "tabSalary Slip" CASCADE;
DROP TABLE IF EXISTS "tabPayroll Entry" CASCADE;
DROP TABLE IF EXISTS "tabSalary Structure Assignment" CASCADE;
DROP TABLE IF EXISTS "tabSalary Structure" CASCADE;
DROP TABLE IF EXISTS "tabEmployee Separation" CASCADE;
DROP TABLE IF EXISTS "tabEmployee" CASCADE;
DROP TABLE IF EXISTS "tabDesignation" CASCADE;
DROP TABLE IF EXISTS "tabDepartment" CASCADE;
DROP TABLE IF EXISTS "tabCompany" CASCADE;
...
CREATE TABLE "tabSalary Slip" (
    ...
    base_gross_pay NUMERIC(18,2),
    base_net_pay NUMERIC(18,2),
    payment_days NUMERIC(10,2),
...
CREATE TABLE "tabLeave Allocation" (
    ...
    from_date DATE,
    to_date DATE,
    new_leaves_allocated NUMERIC(10,2),
    total_leaves_allocated NUMERIC(10,2)
);
...
class PostgresHRSeeder:
    ...
    def run(self):
        print("=" * 60)
        print("PostgreSQL HR Seed Data Generator")
        ...
        self.conn.commit()
        # Force ANALYZE so pg_stat_user_tables has accurate row counts
        print("\n[14/14] Running ANALYZE for accurate statistics...")
...
def main():
    parser = argparse.ArgumentParser(
        description="Seed PostgreSQL with ERPNext-compatible HR test data"
    )
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=5433)
    parser.add_argument("--user", default="hrlab")
    parser.add_argument("--password", default="hrlab")
    ...
    try:
        ...
        print("  docker compose -f targets/postgres-hr/docker-compose.yml up -d")
        sys.exit(1)

    print("Connected successfully.")

    try:
        seeder = PostgresHRSeeder(conn, args.employees, args.years, embed_dq)
        seeder.run()
