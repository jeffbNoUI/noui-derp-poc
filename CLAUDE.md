...
The first target system is **OrangeHRM** (MariaDB). Future targets will be added under `targets/`.
...
- Tests run against the live OrangeHRM database (seeded state)
...
### What "Done" Looks Like for OrangeHRM Target
- [ ] Schema manifest generated (all ~100 tables described)
- [ ] 7 core concept tags correctly assigned (see docs/orangehrm-lab-guide.docx)
...
4. Confirm OrangeHRM is running: `docker compose -f targets/orangehrm/docker-compose.yml ps`
5. Confirm DB is accessible: `mysql -h 127.0.0.1 -P 3306 -u orangehrm -porangehrm -e "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='orangehrm';"`

## Technology

- **Language**: Go 1.21+
- **Target DB (OrangeHRM)**: MariaDB 10.11
...
- Example: `[orangehrm/seed] Add 200-employee seed with DQ issues`
