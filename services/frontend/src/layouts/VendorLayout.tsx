 ...
 * AuthGate redirects to /vendor/login in non-demo mode when unauthenticated.
 * Consumed by: router.tsx (wraps /vendor/* routes)
 * Depends on: ThemeProvider, vendorTheme, VendorAuthProvider, AuthGate, react-router-dom
 */
...
function VendorLayoutInner() {
  ...
  const { vendorId, setVendorId, isAuthenticated, user, logout } = useVendorAuth()
  const navItems = [
  ...
  return (
    <AuthGate isAuthenticated={isAuthenticated} loginPath="/vendor/login">
