...
func (h *Handlers) CalculateRefund(w http.ResponseWriter, r *http.Request) {
	...
	if req.TerminationDate == "" {
		WriteError(w, http.StatusBadRequest, "INVALID_REQUEST", "termination_date is required")
		return
	}
	if req.ApplicationDate == "" {
		WriteError(w, http.StatusBadRequest, "INVALID_REQUEST", "application_date is required")
		return
	}
	if req.HireDate == "" {
		WriteError(w, http.StatusBadRequest, "INVALID_REQUEST", "hire_date is required")
		return
	}

	termDate, err := time.Parse("2006-01-02", req.TerminationDate)
	if err != nil {
		WriteError(w, http.StatusBadRequest, "INVALID_REQUEST", "Invalid termination_date format (use YYYY-MM-DD)")
		return
	}
	appDate, err := time.Parse("2006-01-02", req.ApplicationDate)
	if err != nil {
		WriteError(w, http.StatusBadRequest, "INVALID_REQUEST", "Invalid application_date format (use YYYY-MM-DD)")
		return
	}
	hireDate, err := time.Parse("2006-01-02", req.HireDate)
	if err != nil {
		WriteError(w, http.StatusBadRequest, "INVALID_REQUEST", "Invalid hire_date format (use YYYY-MM-DD)")
		return
	}
