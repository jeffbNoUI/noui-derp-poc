...
func (h *Handlers) ProcessDeath(w http.ResponseWriter, r *http.Request) {
	...
	if req.DeathDate == "" {
		WriteError(w, r, http.StatusBadRequest, "INVALID_REQUEST", "death_date is required")
		return
	}

	deathDate, err := time.Parse("2006-01-02", req.DeathDate)
	if err != nil {
		WriteError(w, r, http.StatusBadRequest, "INVALID_REQUEST", "Invalid death_date format (use YYYY-MM-DD)")
		return
	}
	...
	if req.NotificationDate != "" {
		...
		if err != nil {
			WriteError(w, r, http.StatusBadRequest, "INVALID_REQUEST", "Invalid notification_date format")
			return
		}
	}
...
func (h *Handlers) CalculateSurvivor(w http.ResponseWriter, r *http.Request) {
	...
	if req.MemberBenefit <= 0 {
		WriteError(w, r, http.StatusBadRequest, "INVALID_REQUEST", "member_monthly_benefit must be positive")
		return
	}
	if req.JSPercentage <= 0 || req.JSPercentage > 1.0 {
		WriteError(w, r, http.StatusBadRequest, "INVALID_REQUEST", "js_percentage must be between 0 and 1.0")
		return
	}
	...
	if req.DeathDate != "" {
		...
		deathDate, err = time.Parse("2006-01-02", req.DeathDate)
		if err != nil {
			WriteError(w, r, http.StatusBadRequest, "INVALID_REQUEST", "Invalid death_date format")
			return
		}
	}
