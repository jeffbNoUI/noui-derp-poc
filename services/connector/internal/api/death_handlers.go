...
func (h *Handlers) GetDeathRecord(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		WriteError(w, http.StatusInternalServerError, "INTERNAL_ERROR", "Failed to retrieve death record")
		return
	}
...
func (h *Handlers) GetSurvivorClaims(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		WriteError(w, http.StatusInternalServerError, "INTERNAL_ERROR", "Failed to retrieve survivor claims")
		return
	}
...
func (h *Handlers) PostDeathNotification(w http.ResponseWriter, r *http.Request) {
	...
	if req.NotifySource == "" {
		WriteError(w, http.StatusBadRequest, "INVALID_REQUEST", "notification_source is required")
		return
	}
	...
	if err != nil {
		...
		WriteError(w, http.StatusInternalServerError, "INTERNAL_ERROR", "Failed to save death notification: "+err.Error())
		return
	}
...
func (h *Handlers) PostSurvivorClaim(w http.ResponseWriter, r *http.Request) {
	...
	if deathRec == nil {
		WriteError(w, http.StatusBadRequest, "NO_DEATH_RECORD",
	...
	if req.ClaimType == "" {
		WriteError(w, http.StatusBadRequest, "INVALID_REQUEST", "claim_type is required")
		return
	}
	if req.SurvivorFirst == "" || req.SurvivorLast == "" {
		WriteError(w, http.StatusBadRequest, "INVALID_REQUEST", "survivor name is required")
		return
	}
	...
	if err != nil {
		...
		WriteError(w, http.StatusInternalServerError, "INTERNAL_ERROR", "Failed to save survivor claim: "+err.Error())
		return
	}
