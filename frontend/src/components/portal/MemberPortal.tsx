...
function buildProjectionData(
  ...
  for (let y = startYear; y <= endYear; y += 2) {
    ...
    const conservative = Math.round(conservativeGrowth + futureContributions * (1 + conservativeRate / 2));
    const totalContributed = Math.round(currentBalance * 0.41 + (yearsFromNow > 0 ? annualContribution * 0.41 * yearsFromNow : 0));
...
function StatPill({ label, value, sub, color = C.sage }: {
  label: string; value: string; sub?: string; color?: string;
}) {
  return (
    <div style={{
      background: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 14,
      padding: '18px 22px', transition: 'all 0.3s ease',
    }}>
      <div style={{ fontSize: 11, fontWeight: 500, color: C.textTertiary, marginBottom: 6, letterSpacing: '0.3px' }}>{label}</div>
      <div style={{ fontSize: 28, fontWeight: 700, fontFamily: DISPLAY, color: C.navy, letterSpacing: '-0.5px', lineHeight: 1.1 }}>{value}</div>
...
function buildMilestones(member: { hire_date: string; tier_code: number; dob: string }): Milestone[] {
  ...
  if (member.tier_code <= 2) {
    ...
    const minAge = 55;
    // age + service >= 75
    const ruleYear = Math.max(hireYear + Math.ceil((ruleTarget - (age + svcYears)) + svcYears), birthYear + minAge);
    milestones.push({
...
export default function MemberPortal({
  ...
  if (memberLoading || !effectiveMember) {
    return (
      <div style={{ fontFamily: BODY, background: C.pageBg, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ color: C.textSecondary, fontSize: 16, fontFamily: BODY }}>Loading member data...</div>
        </div>
      </div>
    );
  }
  ...
  const currentBalance = (effectiveContrib?.current_ee_balance ?? 0) + (effectiveContrib?.current_er_balance ?? 0);
  ...
  return (
    <div style={{ fontFamily: BODY, background: C.pageBg, color: C.text, minHeight: '100vh', fontSize: 14, lineHeight: 1.6 }}>
      {/* ═══ TOP NAV ═══ */}
      <div style={{
        background: 'rgba(248, 247, 244, 0.88)', backdropFilter: 'blur(16px)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{
                ...
                background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                color: '#fff', fontFamily: DISPLAY, fontWeight: 700, fontSize: 16,
              }}>N</div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: C.navy, letterSpacing: '-0.3px', fontFamily: DISPLAY, lineHeight: 1.1 }}>NoUI</div>
                <div style={{ fontSize: 9, color: C.textTertiary, letterSpacing: '1.5px', fontWeight: 600, textTransform: 'uppercase' as const }}>Pension Services</div>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button
              ...
              style={{
                ...
                fontFamily: BODY, transition: 'all 0.2s ease',
              }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              }}>{effectiveMember.first_name[0]}{effectiveMember.last_name[0]}</div>
              <div style={{ lineHeight: 1.2 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: C.navy }}>{fullName}</div>
                <div style={{ fontSize: 10, fontFamily: MONO, color: C.textTertiary }}>{memberId}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 1320, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
            <div>
              <div style={{ fontSize: 13, color: C.textOnDarkDim, fontWeight: 500, marginBottom: 6 }}>
                Denver Employees Retirement Plan
              </div>
              <h1 style={{
                fontFamily: DISPLAY, fontSize: 32, fontWeight: 600,
                color: C.textOnDark, letterSpacing: '-0.5px', lineHeight: 1.15, marginBottom: 8,
              }}>
            ...
            <div style={{
              ...
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 30, fontWeight: 700, fontFamily: DISPLAY, color: C.textOnDark }}>
                  {formatCurrency(estimatedMonthly)}
                </div>
                <div style={{ fontSize: 11, color: C.textOnDarkDim, marginTop: 2 }}>Est. Monthly Benefit</div>
              </div>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 11, color: C.textOnDarkDim, marginTop: 2 }}>Years of Service</div>
              </div>
              <div style={{ width: 1, height: 44, background: 'rgba(255,255,255,0.12)' }} />
              <div style={{ textAlign: 'center' }}>
                {vested ? (
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                      ...
                      <span style={{ fontSize: 14, fontWeight: 600, color: '#4ADE80' }}>Fully Vested</span>
                    </div>
                    <div style={{ fontSize: 11, color: C.textOnDarkDim, marginTop: 4 }}>Since {vestingYear(effectiveMember.hire_date)}</div>
                  </>
                ) : (
                  <>
                    <div style={{ fontSize: 14, fontWeight: 600, color: C.gold }}>Not Yet Vested</div>
      ...
      {useDemo && (
        <div style={{
          background: C.goldLight, borderBottom: `1px solid ${C.gold}`,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20, minWidth: 0 }}>
            ...
            <div className="portal-card" style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                <div>
                  <h3 style={{ fontFamily: DISPLAY, fontSize: 18, fontWeight: 600, color: C.navy, marginBottom: 4 }}>Benefit Projection</h3>
                  <p style={{ fontSize: 12, color: C.textTertiary }}>Estimated account growth through normal retirement age</p>
                </div>
            ...
            <div style={{
              ...
              <div className="portal-card" style={{ padding: 24 }}>
                <h3 style={{ fontFamily: DISPLAY, fontSize: 16, fontWeight: 600, color: C.navy, marginBottom: 4 }}>Contribution History</h3>
                <p style={{ fontSize: 12, color: C.textTertiary, marginBottom: 16 }}>Employee + employer annual contributions</p>
                ...
                <div style={{ display: 'flex', gap: 16, marginTop: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: C.textSecondary }}>
                    <div style={{ width: 10, height: 10, borderRadius: 3, background: C.sage, opacity: 0.7 }} /> Employer
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: C.textSecondary }}>
                    <div style={{ width: 10, height: 10, borderRadius: 3, background: C.gold, opacity: 0.7 }} /> Employee
                  </div>
                </div>
              </div>
              ...
              {/* Milestones */}
              <div className="portal-card" style={{ padding: 24 }}>
                <h3 style={{ fontFamily: DISPLAY, fontSize: 16, fontWeight: 600, color: C.navy, marginBottom: 4 }}>Retirement Milestones</h3>
                <p style={{ fontSize: 12, color: C.textTertiary, marginBottom: 20 }}>Key dates on your retirement journey</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                  {milestones.map((ms, i) => (
                    <div key={i} className="portal-milestone-line" style={{ paddingBottom: i < milestones.length - 1 ? 20 : 0 }}>
                      <div style={{
                        ...
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: 9, color: ms.done ? C.sage : C.gold, fontWeight: 700, zIndex: 1,
                      }}>{ms.icon}</div>
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ fontSize: 13, fontWeight: 600, color: ms.done ? C.text : C.navy }}>{ms.label}</span>
                          <span className="portal-tag" style={{
                            background: ms.done ? C.sageLight : C.goldLight,
            ...
            <div className="portal-card" style={{
              ...
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 11, color: C.textTertiary, fontWeight: 500 }}>{item.label}:</span>
                  <span style={{ fontSize: 13, fontWeight: 500, color: C.text }}>{item.value}</span>
                  {i < 2 && <div style={{ width: 1, height: 20, background: C.borderLight, marginLeft: 24 }} />}
                </div>
              ))}
            </div>
            </MotionCard>
            ...
            <div style={{
              ...
              background: C.cardBgWarm, border: `1px solid ${C.borderLight}`,
              textAlign: 'center',
            }}>
              <p style={{ fontSize: 11, fontWeight: 600, color: C.textTertiary, marginBottom: 2 }}>Phase 1: Transparent</p>
...
function MemberMessageCenter({
  ...
  const { data: conversations } = useMemberConversations(contactId);
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: 11, color: C.textTertiary, marginTop: 4 }}>
                  {conv.interactionCount} message{conv.interactionCount !== 1 ? 's' : ''} &middot; {lastUpdated}
                </div>
              </button>
            );
          })}
        ...
        {composing ? (
          <>
            <div style={{
              ...
              <h3 style={{ fontFamily: DISPLAY, fontSize: 16, fontWeight: 600, color: C.navy }}>New Conversation</h3>
              <p style={{ fontSize: 12, color: C.textTertiary, marginTop: 2 }}>Send a secure message to DERP staff</p>
            </div>
            <div style={{ flex: 1 }} />
            <MessageComposer
              theme={MEMBER_THEME}
          ...
          <>
            <div style={{
              ...
              <h3 style={{ fontFamily: DISPLAY, fontSize: 16, fontWeight: 600, color: C.navy }}>
                {convList.find((c) => c.conversationId === effectiveConvId)?.subject || 'Conversation'}
              </h3>
            </div>
  ...
  return (
    <div className="portal-card" style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <div>
          <h3 style={{ fontFamily: DISPLAY, fontSize: 16, fontWeight: 600, color: C.navy, marginBottom: 2 }}>Recent Messages</h3>
          <p style={{ fontSize: 12, color: C.textTertiary }}>Your latest conversations with DERP staff</p>
        </div>
        <button
          onClick={onViewAll}
          style={{
        ...
        {recent.map((conv) => (
          ...
          >
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: C.navy, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>
