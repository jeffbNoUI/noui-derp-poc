...
function renderBoldText(text: string) {
  ...
  return parts.map((part, i) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      return <strong key={i} style={{ fontWeight: 700, color: C.navy }}>{part.slice(2, -2)}</strong>;
    }
    return part;
  });
}
...
export default function AIChatPanel() {
  ...
  return (
    <div style={{
      position: 'sticky', top: 80, height: 'calc(100vh - 108px)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 1 }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: C.sage, animation: 'breathe 2s infinite' }} />
